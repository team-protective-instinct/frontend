        ...
        <Table>
          {isDesktop && (
            <Table.Header>
              ...
              <Table.Head width="w-24" className="text-right">
                Actions
              </Table.Head>
            </Table.Header>
          )}
            ...
            renderItem={(server, idx, isLast) => (
              <Table.Row key={server.id} isLast={isLast}>
                ...
                <Table.Cell>
                  ...
                  <Text className="font-mono text-[10px] text-text-muted">{server.ip}</Text>
                </Table.Cell>
                ...
                {isDesktop && (
                  <>
                    <Table.Cell width="w-48" className="flex-row items-center">
                      <View
                        className={`mr-2 h-2 w-2 rounded-full ${server.agentStatus === 'connected' ? 'bg-threat-safe shadow-[0_0_8px_#00d992]' : 'bg-threat-critical'}`}
                      />
                      <Text
                        className={`text-xs font-bold uppercase tracking-tighter ${server.agentStatus === 'connected' ? 'text-threat-safe' : 'text-threat-critical'}`}>
                ...
                {!isDesktop && (
                  <Table.Cell className="flex-row items-center justify-end">
                    <View
                      className={`h-2 w-2 rounded-full ${server.agentStatus === 'connected' ? 'bg-threat-safe' : 'bg-threat-critical'} mr-4`}
                    />
