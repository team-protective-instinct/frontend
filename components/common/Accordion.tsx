...
export function Accordion({ title, children, isOpenInitial = false }: AccordionProps) {
  ...
  return (
    <View className="border border-[#3d3a39] rounded-lg overflow-hidden bg-[#050507] mb-4">
      {/* 클릭 가능한 헤더 영역 */}
      <TouchableOpacity 
        onPress={toggleAccordion}
        className="flex-row items-center justify-between px-4 py-3 bg-[#101010]"
        activeOpacity={0.7}
      >
        <Text className="text-xs font-bold text-[#f2f2f2] uppercase tracking-widest">{title}</Text>
